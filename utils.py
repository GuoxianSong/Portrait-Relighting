...
from data_utils import ImageFilelist, ImageFolder
import torch
...
def __write_images(image_outputs, display_image_num, file_name,is_test=False):
    if(is_test):
        image_tensor=image_outputs
    else:
...
def test_write_images(image_outputs, display_image_num, file_name):
    image_tensor = image_outputs
    image_grid = vutils.make_grid(image_tensor.data, nrow=display_image_num, padding=0, normalize=True)
    vutils.save_image(image_grid, file_name, nrow=1)
...
def write_image2display(iterations,trainer, train_writer):
    image_dis = torchvision.utils.make_grid(trainer.image_display, nrow=trainer.image_display.size(0) // 2) / 2 + 0.5
